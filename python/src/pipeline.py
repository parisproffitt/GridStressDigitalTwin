...
def main() -> None:
    ...
    # 1) Generate the static transformer grid
    #    (IDs, coordinates, ratings, neighbors)
    nodes = generate_nodes(cfg)

    # 2) Simulate a heatwave and compute per-node load over time
    timesteps = simulate_timesteps(cfg, nodes)

    # 3) Apply rule-based, explainable risk scoring
    #    (risk_score + G/Y/R classification)
    timesteps = apply_risk_model(nodes, timesteps)

    # 4) Generate plain-language explanations for each node state
    timesteps = apply_explanations(nodes, timesteps)

    # 5) Export Unity-ready JSON
    out_path = (
